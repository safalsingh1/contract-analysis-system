...
def process_large_text(text):
    ...
    if not text or not text.strip():
        st.error("Empty text provided. Please check the input document.")
        return pd.DataFrame()  # Return empty DataFrame if text is empty
    ...
    if not all_results:
        st.warning("No valid content found to process.")
        return pd.DataFrame()
